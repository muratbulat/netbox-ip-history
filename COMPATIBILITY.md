...
| 0.3.x | 4.7.x (v4.7.0) | 3.14 | 6.1.0 | VERIFIED | Tested against a real NetBox Community v4.7.0 instance: `manage.py check` (no issues), `makemigrations netbox_ip_history --check --dry-run` (no drift), and a live IP lifecycle (create / reassign / unassign / delete, both via native signals and CSV import) with deleted-object history and global search all verified working. Not yet exercised by `netbox-matrix.yml` CI — see that workflow for the automated matrix. |
...
NetBox 4.6.8 / Python 3.14 / Django 6.0.8 is treated as the priority lane and must stay green above all others. NetBox's own `min_version`/`max_version` bounds (see `netbox_ip_history/__init__.py`) are `4.0.0`–`4.99.99`; only the five rows above have actual evidence behind them — the rest of that range is unverified, not unsupported.
